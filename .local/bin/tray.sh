...
ip addr show wlan0 | grep "inet" > /dev/null && printf "^lm($TERM_COMMAND nmtui)󰖩 ^lm()"
ip addr show eth0 | grep "inet" > /dev/null && printf "^lm($TERM_COMMAND nmtui)󰒍 ^lm()"
printf "^lm($TERM_COMMAND bluetuith) ^lm()"
sunset.sh
printf "/ "
pidof -q dockerd && printf "^lm($HOME lazydocker)󰡨 ^lm()"
pidof -q libvirtd && printf "^lm(virt-manager) ^lm()"
pidof -q cupsd && printf "^lm()󰐪 ^lm()"
pidof -q telegram-desktop && printf "^lm(org.telegram.desktop) ^lm()"
if [[ $(kdeconnect-cli -a  --name-only) == 'pixel7' ]]; then printf "^lm(kdeconnect-app) ^lm()"; fi
pidof -q syncthing && syncthing cli errors show | grep -q ": null" && printf "^lm(xdg-open http://127.0.0.1:8384/) ^lm()" || printf "^lm(xdg-open http://127.0.0.1:8384/)󰓧 ^lm()"
